    ...
    <div style={{minHeight:"100vh",background:"#f9fafb",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:"800px",margin:"2rem auto",padding:"0 1.5rem"}}>
        <div style={{background:"white",borderRadius:"12px",border:"1px solid #e5e7eb",padding:"1.5rem",marginBottom:"1rem"}}>
          ...
          <div style={{height:"1px",background:"#f3f4f6",margin:"1rem 0"}}></div>
        ...
        {result?.security?.length > 0 && (
          <div style={{background:"white",borderRadius:"12px",border:"1px solid #e5e7eb",overflow:"hidden",marginBottom:"1rem"}}>
            <div style={{padding:"10px 16px",background:"#fffbeb",borderBottom:"1px solid #fef3c7",fontSize:"13px",fontWeight:500,color:"#92400e"}}>🔒 Security issues ({result.security.length})</div>
            <div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:"8px"}}>
              {result.security.map((s: any, i: number) => (
                <div key={i} style={{background:"#fffbeb",borderRadius:"8px",padding:"10px 12px",fontSize:"13px"}}>
                  <strong style={{color:"#92400e"}}>{s.issue}</strong>
                  {s.fix && <div style={{color:"#6b7280",marginTop:"3px"}}>Fix: {s.fix}</div>}
                </div>
              ))}
            </div>
          </div>
